...
async def lottery(orders: List[Dict[str, Any]]) -> List[str]:
    ...
    return lottery_results_resolver(lottery_results)
...
def lottery_results_resolver(results: List[Dict[str, Any]]) -> List[str]:
    ...
    for result in results:
        ...
        if len(cards) == 0:
            ...
            # 本次积分: {积分} 获得卡牌:
            # {卡牌名称}×{数量}
            card_with_pic = random.choice(cards)
            score = result['score']
            cards_result = cards_data_resolver(cards)
            msg = templates.lottery_result_template(card_with_pic,
                                                    cards_result, score)
            msgs.append(msg)
    return msgs
...
def cards_data_resolver(cards: List[tuple]) -> str:
    ...
    for card in cards:
        ...
        if name not in cards_dict:
            ...
            cards_dict[name] += 1
    return templates.card_template(cards_dict)
